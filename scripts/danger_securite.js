...
const outFile = path.join(outDir, 'generated_danger_securite.md');
...
function processFile(filePath) {
    ...
    // match variants: Danger sécurité / Danger/Sécurité / Danger securite (case-insensitive, accents optional)
    const dangerRegex = /^(#{1,6}\s*)?Danger[\s\/]?(sécurit[eé]|securite|sécurité)\s*$/i;
    const idx = findSectionIndex(lines, dangerRegex);
...
function main() {
    ...
    let out = '# Generated Danger / Sécurité (extracted from Chapters)\n\n';
    out += `> Generated on ${new Date().toISOString()} by scripts/danger_securite.js\n\n`;
    if (!results.length) out += '_No Danger/Sécurité sections found in Chapters/_\n';
    else {
